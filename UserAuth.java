...
import javax.swing.*;
class UserAuth extends JFrame implements ActionListener,FocusListener
{
...
t1=new JTextField();
graph = new JButton("View Graph");
next = new JButton("Get Data");
t2=new JTextField();
l3.setBounds(120,70,100,30);
l3.setFont(new Font("Times New Roman", Font.BOLD, 18));
add(l3);
l4.setBounds(215,70,400,30);
...
add(b3);
graph.setBounds(420,300,100,30);
add(graph);
graph.setVisible(false);
b1.setBounds(245,250,100,30);
add(b1);
next.setBounds(420,130,100,30);
add(next);
b2.setBounds(420,250,150,30);
add(b2);
b1.setEnabled(false);
b2.setVisible(false);
l2.setVisible(false);
...
if(ae.getActionCommand()=="Check")
{
		b2.setVisible(false);
		//try{
		/*	ObjectInputStream in = new ObjectInputStream(new FileInputStream("C:\\Users\\Kaustubh\\Desktop\\MyKData\\"+t1.getText()+".txt"));
        String allkeystrokestore[][][]=new String[12][30][8];
...
System.out.println("True |Key - "+allkeystrokestore[0][i][0]+"|MeanS ="+allkeystrokestore[10][i][j]+"|std="+allkeystrokestore[11][i][j]+"|Ct"+keystore[i][j]);
			...
			System.out.println("False| Key - "+allkeystrokestore[0][i][0]+"|MeanS ="+allkeystrokestore[10][i][j]+"|std="+allkeystrokestore[11][i][j]+"|Ct"+keystore[i][j]);
				...
				int fare = result+40;
				System.out.println("% Match With Stored Keystroke Dynamics = "+result);
				if(result>=70)
				{
					b2.setText("REAL USER");
...
if(ae.getActionCommand()=="Get Data"){
	...
	allkeystrokestore= (String[][][]) in.readObject();
        in.close();
	}catch(Exception e){System.out.println("Error in next button");}

	password = allkeystrokestore[12][0][0];
...
if(ae.getActionCommand()=="View Graph"){

new GetGraphData(allkeystrokestore, keystore,passLength);
